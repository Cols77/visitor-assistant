...
def embed_texts(texts: Iterable[str]) -> List[List[float]]:
    ...
    for text in texts:
        ...
        expected_dims = config.settings.embedding_dims
        cached = _cached_embedding(text_hash)
        if cached is not None:
            if len(cached) == expected_dims:
                vectors.append(cached)
                continue
            # Cache dimension is stale; fall through to recompute and overwrite.
        if not config.settings.llm_api_key:
            vector = _deterministic_embedding(text, expected_dims)
